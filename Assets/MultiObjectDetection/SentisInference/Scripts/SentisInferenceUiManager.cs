...
namespace PassthroughCameraSamples.MultiObjectDetection
{
    ...
    public class SentisInferenceUiManager : MonoBehaviour
    {
        ...
        [SerializeField] private GameObject m_specialObject;        // 指定類別為 'a' 時要跳出的物件
        private bool m_hasShownSpecialObject = false;
        ...
        #region 跳出object
        private void SpawnSpecialObject(Vector3 position)
        {
            if (m_specialObject != null)
            {
                Instantiate(m_specialObject, position, Quaternion.identity);
            }
        }
        ...
        #region label添加文字
        //添加茶壺
        private readonly Dictionary<string, string> m_classNameTranslations = new()
        {
            { "tea_pot", "茶壺" },
            { "tea_bowl", "茶海" },
        ...
        public void DrawUIBoxes(Tensor<float> output, Tensor<int> labelIDs, float imageWidth, float imageHeight)
        {
            ...
            if (boxesFound <= 0)
            {
                ...
                if (m_specialObject != null)
                {
                    m_specialObject.SetActive(false); // 無辨識時隱藏
                }
                //
                return;
            ...
            {
                //
                var classRaw = m_labels[labelIDs[n]].Replace(" ", "_");
                var classTranslated = m_classNameTranslations.ContainsKey(classRaw)
                    ? $"{classRaw} {m_classNameTranslations[classRaw]}"
                    : classRaw;
                //
                var box = new BoundingBox
                {
                    ...
                    //
                    //Label = $"ID: {n} CLASS: {classTranslated}",
                    Label = classTranslated ,
                    //
                    PerX = 0.0f,
                    PerY = 0.0f,
                ...
                //最後判斷是否要顯示提示物件
                bool hasSpecialClass = BoxDrawn.Exists(b => b.ClassName == "tea_pot");

                if (m_specialObject != null)
                {
                    if (hasSpecialClass && !m_hasShownSpecialObject)
                    {
                        m_specialObject.SetActive(true);
                        m_hasShownSpecialObject = true; //  只顯示一次
                        m_specialObject.transform.position = new Vector3(0f, 1f, 1f);
                    }
                }
